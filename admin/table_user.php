        ...
        <form class="modal-content" action="update_user.php" method="post" enctype="multipart/form-data">
          ...
          <div class="modal-body">
            ...
            <select name="kelasid" id="kelasid" class="form-control mb-2">
              ...
              // Mengambil nilai kelasid_wali dari $data
              $kelasid_wali = $data['kelasid'];

              // Query untuk memeriksa apakah kelasid_wali ada dalam tabel kelas
              $sql_kelas_wali = mysqli_query($conn, "SELECT * FROM kelas WHERE kelasid='$kelasid_wali'");

              // Jika kelasid dalam $data adalah 0, tampilkan opsi "Tidak Ada" dan semua kelas
              if ($data['kelasid'] == 0) {
              ?>
                ...
                // Jika kelasid_wali tidak ditemukan dalam tabel kelas, tampilkan "Data Error"
                ?>
            ...
            <select name="jurusanid" id="jurusanid" class="form-control mb-2">
              ...
              // Mengambil nilai kelasid_wali dari $data
              $jurusanid_wali = $data['jurusanid'];

              // Query untuk memeriksa apakah kelasid_wali ada dalam tabel kelas
              $sql_jurusan_wali = mysqli_query($conn, "SELECT * FROM jurusan WHERE jurusanid='$jurusanid_wali'");
              ...
              ?>
                ...
                ?>
                  <option value="<?= $data_jurusan['jurusanid'] ?>"><?= $data_jurusan['kepanjangan'] ?></option>
                <?php
                }
              } else if (mysqli_num_rows($sql_jurusan_wali) == 0) {
                // Jika jurusanid_wali tidak ditemukan dalam tabel kelas, tampilkan "Data Error"
                ?>
                <option value="" selected>Data Error</option>
                <?php
                while ($data_jurusan = mysqli_fetch_array($sql_jurusan)) {
                ...
                ?>
                  <option value="<?= $data_jurusan['jurusanid'] ?>" <?php if ($data_jurusan['jurusanid'] == $data['jurusanid']) {
                                                                      echo 'selected';
                                                                    } ?>><?= $data_jurusan['kepanjangan'] ?></option>
                <?php
                }
                ?>
                <option value="">Tidak Ada</option>
      ...
      <form class="modal-content" action="update_profile.php" method="post" enctype="multipart/form-data">
        ...
        <div class="modal-body">
          ...
          ?>
            ...
            <input type="password" class="form-control mb-2" id="password-profile" name="password" placeholder="klik dan ketik disini..." value="<?= $data['password'] ?>">
