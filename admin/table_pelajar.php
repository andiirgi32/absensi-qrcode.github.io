...
<head>
  ...
  <title>Ruang<?= $_SESSION['role'] ?> - Table Semua Pelajar</title>
...
<body id="page-top">
        ...
        <li class="nav-item">
          ...
          <div id="collapseTable" class="collapse show" aria-labelledby="headingTable" data-parent="#accordionSidebar">
            <div class="card py-2 collapse-inner rounded">
              ...
              <a class="collapse-item active btn-light" href="table_pelajar.php">Table Pelajar</a>
      ...
      <li class="nav-item">
        ...
        <div id="collapseTablePelajar" class="collapse" aria-labelledby="headingTable" data-parent="#accordionSidebar">
          <div class="card py-2 collapse-inner rounded">
            <h6 class="collapse-header">Tables</h6>
            <?php
            $selectedKelasid = isset($_GET['kelasid']) ? $_GET['kelasid'] : '';
            if ($_SESSION['role'] == "Admin" || $_SESSION['role'] == "Kepala Sekolah") {
              $sql_kelas = mysqli_query($conn, "SELECT * FROM kelas");
              while ($data_kelas = mysqli_fetch_array($sql_kelas)) {
                $activeClass = ($data_kelas['kelasid'] == $selectedKelasid) ? 'active btn-light' : '';
            ?>
                <a class="collapse-item <?= $activeClass ?>" href="data_siswa_kelas.php?kelasid=<?= $data_kelas['kelasid'] ?>&namakelas=<?= $data_kelas['namakelas'] ?>&jurusanid=<?= $data_kelas['jurusanid'] ?>">Table <?= $data_kelas['namakelas'] ?></a>
              <?php
              }
            } else if ($_SESSION['role'] == "Wali Kelas") {
      ...
      <div id="content">
        ...
        <div class="container-fluid bg-body-tertiary" id="container-wrapper">
          <div class="d-sm-flex align-items-center justify-content-between mb-4">
            <h1 class="h3 mb-0">DataTable Semua Pelajar</h1>
            <ol class="breadcrumb">
              <li class="breadcrumb-item"><a href="./">Home</a></li>
              <li class="breadcrumb-item" aria-current="page">Tables</li>
              <li class="breadcrumb-item active" aria-current="page">DataTable Semua Pelajar</li>
            </ol>
          </div>
            ...
            <div class="col-lg-12">
              <div class="card mb-4">
                <div class="card-header py-3 d-flex flex-row align-items-center justify-content-between">
                  <h6 class="m-0 font-weight-bold text-primary">Table Semua Pelajar</h6>
                  <a href="" data-bs-toggle="modal" data-bs-target="#tambahPelajar" class="btn btn-primary btn-sm"><i class="bi bi-file-earmark-plus-fill"></i></a>
                </div>
                <div class="table-responsive p-3">
                  <table class="table align-items-center table-flush table-striped table-hover" id="dataTable">
                    ...
                    <tbody>
                      <?php
                      $sql = mysqli_query($conn, "SELECT siswa.*, jurusan.*, kelas.namakelas FROM siswa INNER JOIN jurusan ON siswa.jurusanid=jurusan.jurusanid INNER JOIN kelas ON siswa.kelasid=kelas.kelasid ORDER BY siswa.nama ASC");
                      $no = 1;
                      ...
                      ?>
                        <tr>
                          ...
                          <td><?= $data['jk'] ?></td>
                          <td>
                            <a href="" data-bs-toggle="modal" data-bs-target="#editPelajar<?= $data['idsiswa'] ?>" class="btn btn-warning mb-1"><i class="bi bi-pencil-square"></i></a>
                            <form action="hapus_siswa.php?idsiswa=<?= $data['idsiswa'] ?>" method="post" onsubmit="return confirm('Apakah kamu yakin ingin menghapusnya?')" style="display: inline;">
  ...
  <!-- Modal Scrollable -->
  <div class="modal fade" id="tambahPelajar" tabindex="-1" role="dialog" aria-labelledby="exampleModalScrollableTitle" aria-hidden="true">
    <div class="modal-dialog modal-dialog-scrollable modal-lg" role="document">
      <form class="modal-content" action="proses_siswa.php" method="post" enctype="multipart/form-data">
        <div class="modal-header">
          <h5 class="modal-title" id="exampleModalScrollableTitle">Tambah Peserta</h5>
          <button type="button" class="close" data-bs-dismiss="modal" aria-label="Close">
            <span aria-hidden="true">&times;</span>
          </button>
          <!-- <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button> -->
        ...
        <div class="modal-body">
          ...
          <select name="kelasid" id="kelasid" class="form-control mb-2" required>
            ...
            ?>
              <option value="<?= $data_kelas_2['kelasid'] ?>"><?= $data_kelas_2['namakelas'] ?></option>
            <?php
            }
            ?>
          </select>
          ...
          <select name="jurusanid" id="jurusanid" class="form-control mb-2" required>
            ...
            ?>
              <option value="<?= $data_jurusan_2['jurusanid'] ?>"><?= $data_jurusan_2['kepanjangan'] ?></option>
            <?php
            }
            ?>
          </select>
  ...
  ?>
    <!-- Modal Scrollable -->
    <div class="modal fade" id="editPelajar<?= $data['idsiswa'] ?>" tabindex="-1" role="dialog" aria-labelledby="exampleModalScrollableTitle" aria-hidden="true">
      <div class="modal-dialog modal-dialog-scrollable modal-lg" role="document">
        <form class="modal-content" action="update_siswa.php" method="post" enctype="multipart/form-data">
          <div class="modal-header">
            <h5 class="modal-title" id="exampleModalScrollableTitle">Edit Peserta</h5>
            <button type="button" class="close" data-bs-dismiss="modal" aria-label="Close">
              <span aria-hidden="true">&times;</span>
            </button>
          </div>
        ...
        <div class="modal-body">
          ...
          ?>
            ...
            <label for="inputFotoUser" style="display: block;" class="mb-3">
              <img src="fotouser/<?= $data['fotouser'] ?>" alt="" class="berita-user-profile" id="imgFotoUser">
            </label>
